...
class Loader {
  ...
  countResources() {
    ...
    this.resources.total =
      images.length + videos.length + stylesheets.length + scripts.length;
  }
  ...
  animateCounter() {
    const updateCounter = () => {
      ...
      const timeProgress = Math.min(
        (Date.now() - this.startTime) / (3000 / this.speedMultiplier),
        0.8,
      );
      const resourceProgress =
        this.resources.total > 0
          ? (this.resources.loaded / this.resources.total) * 0.6
          : 0.3;
      const pageProgress = this.isPageLoaded ? 0.2 : 0;

      const targetProgress =
        (timeProgress + resourceProgress + pageProgress) * 100;
